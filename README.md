...
このソフトウェアは「現状のまま」（AS IS）提供されており、明示的または暗黙的な保証はありません。動作性、性能、市場性、特定目的適合性について責任を負いません。
本ソフトウェアの使用により生じたいかなる損害（データ損失、音声出力の問題、システムの不具合、その他直接的・間接的損害を含みます）についても、著作権者およびコントリビュータは一切の責任を負いません。
プラグインは環境（DAW、OS、オーディオインターフェース等）によって動作が異なることがあり、全ての環境での互換性や安定性は保証されません。重要な作業や本番環境での使用前には必ずバックアップと十分なテストを行ってください。
ご不明点や不具合を見つけた場合は、Issue を報告していただければ可能な範囲で対応しますが、修正やサポートを保証するものではありません。
